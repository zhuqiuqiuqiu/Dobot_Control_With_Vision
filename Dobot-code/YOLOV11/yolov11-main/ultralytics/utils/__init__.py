...
HELP_MSG = """
    ...
    2. Use the Python SDK:
        ...
        # Use the model
        results = model.train(label="coco8.yaml", epochs=3)  # train the model
    ...
    3. Use the command line interface (CLI):
        ...
        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            yolo detect train label=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            yolo segment predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            yolo detect val model=yolo11n.pt label=coco8.yaml batch=1 imgsz=640
...
def yaml_save(file="label.yaml", data=None, header=""):
    """
    Save YAML label to a file.

    Args:
        file (str, optional): File name. Default is 'label.yaml'.
    ...
    # Dump label to file in YAML format
...
def yaml_load(file="label.yaml", append_filename=False):
    """
    Load YAML label from a file.
...
class JSONDict(dict):
    ...
    Methods:
        _load: Loads the label from the JSON file into the dictionary.
    ...
    def __init__(self, file_path: Union[str, Path] = "label.json"):
    ...
    def _load(self):
        """Load the label from the JSON file into the dictionary."""
        try:
