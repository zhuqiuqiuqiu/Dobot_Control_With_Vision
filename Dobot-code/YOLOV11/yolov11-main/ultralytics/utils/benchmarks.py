...
class RF100Benchmark:
        ...
        with open(ds_link_txt) as file:
            for line in file:
                try:
                    ...
                    self.ds_cfg_list.append(Path.cwd() / proj_version / "label.yaml")
                except Exception:
                    continue
    ...
    def fix_yaml(path):
        ...
        Examples:
            >>> RF100Benchmark.fix_yaml("path/to/label.yaml")
        """
    ...
    def evaluate(self, yaml_path, val_log_file, eval_log_file, list_ind):
        ...
        Examples:
            ...
            >>> benchmark = RF100Benchmark()
            >>> benchmark.evaluate("path/to/label.yaml", "path/to/val_log.txt", "path/to/eval_log.txt", 0)
        """
        skip_symbols = ["🚀", "⚠️", "💡", "❌"]
...
class ProfileModels:
    ...
    def iterative_sigma_clipping(data, sigma=2, max_iters=3):
        """Applies iterative sigma clipping to label to remove outliers based on specified sigma and iteration count."""
        data = np.array(data)
