...
def run_ray_tune(
    ...
    Example:
        ...
        result_grid = model.tune(label="coco8.yaml", use_ray=True)
        ```
    """
    ...
    # Get dataset
    data = train_args.get("label", TASK2DATA[task])
    space["label"] = data
    if "label" not in train_args:
        LOGGER.warning(f'WARNING ⚠️ label not provided, using default "label={data}".')
