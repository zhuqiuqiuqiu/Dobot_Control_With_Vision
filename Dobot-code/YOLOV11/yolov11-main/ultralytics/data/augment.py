...
class BaseTransform:
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image label and annotations. Expected keys include 'img' for
                the image label, and 'instances' for object instances.
...
class Compose:
    ...
    Methods:
        __call__: Applies a series of transformations to input label.
    ...
    Examples:
        ...
        >>> transformed_data = compose(label)
    ...
    def __init__(self, transforms):
        ...
        Examples:
            >>> from ultralytics.label.augment import Compose, RandomHSV, RandomFlip
    ...
    def __call__(self, data):
        """
        Applies a series of transformations to input label. This method sequentially applies each transformation in the
        Compose object's list of transforms to the input label.

        Args:
            data (Any): The input label to be transformed. This can be of any type, depending on the
                transformations in the list.

        Returns:
            (Any): The transformed label after applying all transformations in sequence.
...
class BaseMixTransform:
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        Examples:
            >>> dataset = YOLODataset("path/to/label")
    ...
    def __call__(self, labels):
        """
        Applies pre-processing transforms and mixup/mosaic transforms to labels label.
        ...
        Args:
            labels (Dict): A dictionary containing label label for an image.

        Returns:
            (Dict): The transformed labels dictionary, which may include mixed label from other images.
    ...
    def _mix_transform(self, labels):
        ...
        Mosaic. It modifies the input label dictionary in-place with the augmented label.

        Args:
            labels (Dict): A dictionary containing image and label label. Expected to have a 'mix_labels' key
                with a list of additional image and label label for mixing.

        Returns:
            (Dict): The modified labels dictionary with augmented label after applying the mix transform.
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from ultralytics.label.augment import Mosaic
    ...
    def _mix_transform(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image label and annotations. Expected keys include:
                - 'rect_shape': Should be None as rect and mosaic are mutually exclusive.
                - 'mix_labels': A list of dictionaries containing label for other images to be used in the mosaic.
    ...
    def _mosaic4(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image label and labels for the base image (index 0) and three
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from ultralytics.label.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        Examples:
            >>> from ultralytics.label.dataset import YOLODataset
            >>> dataset = YOLODataset("path/to/label.yaml")
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image label and annotations.
                Must include:
...
class RandomHSV:
    ...
    Examples:
        >>> import numpy as np
        >>> from ultralytics.label.augment import RandomHSV
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image label and metadata. Must include an 'img' key with
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        Args:
            labels (Dict | None): A dictionary containing image label and associated labels, or empty dict if None.
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from ultralytics.label.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
...
class Albumentations:
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image label and annotations. Expected keys are:
...
class Format:
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing image and annotation label with the following keys:
                - 'img': The input image as a numpy array.
                - 'cls': Class labels for instances.
                - 'instances': An Instances object containing bounding boxes, segments, and keypoints.

        Returns:
            (Dict): A dictionary with formatted label, including:
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    Args:
        dataset (Dataset): The dataset object containing image label and annotations.
    ...
    if dataset.use_keypoints:
        ...
        if len(flip_idx) == 0 and hyp.fliplr > 0.0:
            hyp.fliplr = 0.0
            LOGGER.warning("WARNING ⚠️ No 'flip_idx' array defined in label.yaml, setting augmentation 'fliplr=0.0'")
        elif flip_idx and (len(flip_idx) != kpt_shape[0]):
            raise ValueError(f"label.yaml flip_idx={flip_idx} length must be equal to kpt_shape[0]={kpt_shape[0]}")
