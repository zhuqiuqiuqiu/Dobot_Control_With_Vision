...
CLI_HELP_MSG = f"""
    ...
    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo train label=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        yolo predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Val a pretrained detection model at batch-size 1 and image size 640:
        yolo val model=yolo11n.pt label=coco8.yaml batch=1 imgsz=640

    4. Export a YOLO11n classification model to ONNX format at image size 224 by 128 (no TASK required)
        yolo export model=yolo11n-cls.pt format=onnx imgsz=224,128

    5. Explore your datasets using semantic search and SQL with a simple GUI powered by Ultralytics Explorer API
        yolo explorer label=label.yaml model=yolo11n.pt
...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None):
    """
    Load and merge configuration label from a file or dictionary, with optional overrides.

    Args:
        cfg (str | Path | Dict | SimpleNamespace): Configuration label source. Can be a file path, dictionary, or
    ...
    Notes:
        ...
        - The function performs type and value checks on the configuration label.
    """
    cfg = cfg2dict(cfg)
...
def handle_explorer(args: List[str]):
    ...
    cmd = ["streamlit", "run", ROOT / "label/explorer/gui/dash.py", "--server.maxMessageSize", "2048"]
    new = dict(parse_key_value_pair(a) for a in args)
    check_dict_alignment(base={k: DEFAULT_CFG_DICT[k] for k in ["model", "label"]}, custom=new)
...
def entrypoint(debug=""):
    ...
    Examples:
        ...
        >>> entrypoint("train label=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01")

        Predict a YouTube video using a pretrained segmentation model at image size 320:
        >>> entrypoint("predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320")

        Validate a pretrained detection model at batch-size 1 and image size 640:
        >>> entrypoint("val model=yolo11n.pt label=coco8.yaml batch=1 imgsz=640")
    ...
    if mode in {"predict", "track"} and "source" not in overrides:
        ...
        if "label" not in overrides and "resume" not in overrides:
            overrides["label"] = DEFAULT_CFG.data or TASK2DATA.get(task or DEFAULT_CFG.task, DEFAULT_CFG.data)
            LOGGER.warning(f"WARNING ⚠️ 'label' argument is missing. Using default 'label={overrides['label']}'.")
    elif mode == "export":
