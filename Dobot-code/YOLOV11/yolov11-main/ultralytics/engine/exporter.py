...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if self.args.int8 and not self.args.data:
            self.args.data = DEFAULT_CFG.data or TASK2DATA[getattr(model, "task", "detect")]  # assign default label
            LOGGER.warning(
                "WARNING ⚠️ INT8 export requires a missing 'label' arg for calibration. "
                f"Using default 'label={self.args.data}'."
            )
        # Input
        ...
        data = model.args["label"] if hasattr(model, "args") and isinstance(model.args, dict) else ""
        ...
        if any(f):
            ...
            predict_data = f"label={data}" if model.task == "segment" and fmt == "pb" else ""
            q = "int8" if self.args.int8 else "half" if self.args.half else ""  # quantization
            LOGGER.info(
                f'\nExport complete ({time.time() - t:.1f}s)'
                f"\nResults saved to {colorstr('bold', file.parent.resolve())}"
                f'\nPredict:         yolo predict task={model.task} model={f} imgsz={imgsz} {q} {predict_data}'
                f'\nValidate:        yolo val task={model.task} model={f} imgsz={imgsz} label={data} {q} {s}'
    ...
    def get_int8_calibration_dataloader(self, prefix=""):
        ...
        LOGGER.info(f"{prefix} collecting INT8 calibration images from 'label={self.args.data}'")
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        if self.args.int8:
            ...
            # Generate calibration label for integer quantization
            ignored_scope = None
    ...
    def export_engine(self, prefix=colorstr("TensorRT:")):
        ...
        if int8:
            ...
            class EngineCalibrator(trt.IInt8Calibrator):
                def __init__(
                    self,
                    dataset,  # ultralytics.label.build.InfiniteDataLoader
                    batch: int,
                    cache: str = "",
                ) -> None:
                ...
                def get_batch(self, names) -> list:
                    ...
                    try:
                        ...
                        # Return [] or None, signal to TensorRT there is no calibration label remaining
                        return None
