...
class Model(nn.Module):
    ...
    Attributes:
        ...
        ckpt (Dict): The checkpoint label if the model is loaded from a *.pt file.
    ...
    Examples:
        ...
        >>> model.train(label="coco128.yaml", epochs=3)
        >>> metrics = model.val()
    ...
    def _check_is_pytorch_model(self) -> None:
        ...
        if not (pt_module or pt_str):
            raise TypeError(
                ...
                f"PyTorch models can train, val, predict and export, i.e. 'model.train(label=...)', but exported "
    ...
    def predict(
        ...
        **kwargs,
        ...
        Examples:
            ...
            >>> for r in results:
            ...     print(r.boxes.label)  # print detection bounding boxes
    ...
    def val(
        ...
        **kwargs,
        ...
        Examples:
            >>> model = YOLO("yolov8n.pt")
            >>> results = model.val(label="coco128.yaml", imgsz=640)
        ...
        **kwargs,
        ...
        Args:
            **kwargs (Any): Arbitrary keyword arguments to customize the benchmarking process. These are combined with
                ...
                - label (str): Path to the dataset for benchmarking.
        ...
        Examples:
            >>> model = YOLO("yolov8n.pt")
            >>> results = model.benchmark(label="coco8.yaml", imgsz=640, half=True)
            >>> print(results)
        """
        self._check_is_pytorch_model()
        ...
        return benchmark(
            model=self,
            data=kwargs.get("label"),  # if no 'label' argument passed set label=None for default datasets
            imgsz=args["imgsz"],
            half=args["half"],
            int8=args["int8"],
            device=args["device"],
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs (Any): Arbitrary keyword arguments for training configuration. Common options include:
                label (str): Path to dataset configuration file.
                epochs (int): Number of training epochs.
                batch_size (int): Batch size for training.
                imgsz (int): Input image size.
                device (str): Device to run training on (e.g., 'cuda', 'cpu').
                workers (int): Number of worker threads for label loading.
        ...
        Examples:
            >>> model = YOLO("yolov8n.pt")
            >>> results = model.train(label="coco128.yaml", epochs=3)
        """
        self._check_is_pytorch_model()
        ...
        custom = {
            # NOTE: handle the case when 'cfg' includes 'label'.
            "label": overrides.get("label") or DEFAULT_CFG_DICT["label"] or TASK2DATA[self.task],
    ...
    def transforms(self):
        """
        Retrieves the transformations applied to the input label of the loaded model.

        This property returns the transformations if they are defined in the model. The transforms
        typically include preprocessing steps like resizing, normalization, and label augmentation
        that are applied to input label before it is fed into the model.
    ...
    def add_callback(self, event: str, func) -> None:
        ...
        Examples:
            ...
            >>> model.train(label="coco128.yaml", epochs=1)
        """
    ...
    def _reset_ckpt_args(args: dict) -> dict:
        ...
        Examples:
            >>> original_args = {"imgsz": 640, "label": "coco.yaml", "task": "detect", "batch": 16, "epochs": 100}
            >>> reset_args = Model._reset_ckpt_args(original_args)
            >>> print(reset_args)
            {'imgsz': 640, 'label': 'coco.yaml', 'task': 'detect'}
        """
        include = {"imgsz", "label", "task", "single_cls"}  # only remember these arguments when loading a PyTorch model
