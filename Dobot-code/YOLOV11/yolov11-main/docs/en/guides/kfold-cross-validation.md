...
2. Proceed to retrieve all label files for your dataset.
    ...
    dataset_path = Path("./Fruit-detection")  # replace with 'path/to/dataset' for your custom label
    labels = sorted(dataset_path.rglob("*labels/*.txt"))  # all label in 'labels'
    ```

3. Now, read the contents of the dataset YAML file and extract the indices of the class labels.

    ```python
    yaml_file = "path/to/label.yaml"  # your label YAML with label directories and names dictionary
