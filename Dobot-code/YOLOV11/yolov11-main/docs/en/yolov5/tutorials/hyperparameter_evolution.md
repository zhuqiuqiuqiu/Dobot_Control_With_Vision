...
# python train.py --batch 64 --cfg yolov5n6.yaml --weights '' --label coco.yaml --img 640 --epochs 300 --linear
...
```bash
python train.py --epochs 10 --label coco128.yaml --weights yolov5s.pt --cache
```
...
```bash
# Single-GPU
python train.py --epochs 10 --label coco128.yaml --weights yolov5s.pt --cache --evolve

# Multi-GPU
for i in 0 1 2 3 4 5 6 7; do
  sleep $(expr 30 \* $i) &&  # 30-second delay (optional)
  echo 'Starting GPU '$i'...' &&
  nohup python train.py --epochs 10 --label coco128.yaml --weights yolov5s.pt --cache --device $i --evolve > evolve_gpu_$i.log &
done
