...
DEFAULT_MODEL_PATH = r"E:\best.pt"
DEFAULT_CALIB_XML = r"E:\2-5\2-5 Vision\机械臂标定工具.xml"
...
class Setting:
    ...
    place_targets = {
        "WIFI": {"x": 291.93, "y": 155.59, "z": -164.35},
        "CHIP": {"x": 265.75, "y": 156.19, "z": -164.35},
        "GPS": {"x": 316.16, "y": 154.13, "z": -164.35},
        "Square_silver": {"x": 316.48, "y": 124.11, "z": -164.35},
        "Square_yellow": {"x": 291.36, "y": 123.83, "z": -164.35},
        "Square_red": {"x": 266.59, "y": 124.93, "z": -164.35},
...
class DobotClient:
    ...
    def vision_pick(self, x, y, r=None, r_delta=0.0, target_name=None):
        ...
        print(f"Vision pick target: name={target_name}, X={x:.3f}, Y={y:.3f}, R={r:.3f}")
        ...
        if place_x is not None and place_y is not None:
            ...
            place_r = r
            print(f"Move to place: name={target_name}, X={place_x:.3f}, "
...
class VisionPanel:
    ...
    def load_model(self):
        ...
        try:
            ...
            print(f"YOLO model loaded: {self.model_path.get().strip()}")
...
##### 修改这里
                    ...
                    # )
                    print(f"已完成抓取 {success_count}/{Setting.Count}")
    ...
    def _pick_from_detection(self, detection, angle=0):
        ...
        # )
        print(            f"Image ({cx:.1f}, {cy:.1f}) -> Robot ({robot_x:.3f}, {robot_y:.3f})")
#### 5/28 修改位姿补偿
...
#         self.log(f"R relative correction delta = {angle:.3f}")
        print(f"R relative correction delta = {angle:.3f}")
