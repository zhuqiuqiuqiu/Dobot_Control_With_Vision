...
def get_pca_angle_from_frame(img, debug=False):
    """
    输入：OpenCV frame
    输出：PCA主方向角（degree）
    这里是输出视觉角度的
...
class DobotClient:
    ...
    def vision_pick(self, x, y, r=None):
        self.require_connected()
        r = Setting.pause_pick if r is None else r
        self.logger(f"Vision pick target: X={x:.3f}, Y={y:.3f}, R={r:.3f}")
        ...
        self.move.MovL(Setting.place_x, Setting.place_y,Setting.place_z, r)
        time.sleep(0.1)
        self.dashboard.DO(Setting.do1, Setting.do1_no)
        self.wait_move_done()

        if Setting.place_x is not None and Setting.place_y is not None:
            place_z = Setting.place_z if Setting.place_z is not None else Setting.safe_high
            place_r = Setting.place_r if Setting.place_r is not None else r
            self.logger(f"Move to place: X={Setting.place_x:.3f}, Y={Setting.place_y:.3f}")
            self.move.MovJ(Setting.place_x, Setting.place_y, place_z, place_r)
            self.wait_move_done()
            self.dashboard.DO(Setting.do1, Setting.do1_no)
            time.sleep(0.1)
    ...
    def get_current_r(self):
        ...
        if start == -1 or end == -1:
            raise ValueError("GetPose parse failed")
...
##### 新加入 5/28 加入位姿补偿
                angle = 0
                try:
                    angle = get_pca_angle_from_frame(frame)
...
#  需要修改Setting.pause_pick为实时的r角度
        current_r = self.client.get_current_r()
        # PCA 返回的是当前物体姿态相对视觉基准的修正量，
        # 实际下发给机器人时要叠加到当前末端 R 上。
        r = normalize_angle_deg(current_r + angle)
        self.log(
            f"Current R = {current_r:.3f}, angle delta = {angle:.3f}, target R = {r:.3f}"
        )
        self.client.vision_pick(robot_x, robot_y, r=r)
